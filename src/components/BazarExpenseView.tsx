...
import { CurryExpense } from "../types";
...
export const BazarExpenseView: React.FC = () => {
  const { user } = useAuth();
  const { activeMonth, isMonthClosed, users, notify, refreshData } = useMess();

  const [expenses, setExpenses] = useState<CurryExpense[]>([]);
  ...
  const loadExpenses = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await fetch(`/api/months/${activeMonth.id}/curry-expenses`);
      if (res.ok) {
        const data = await res.json();
        setExpenses(data);
      }
    } catch (e) {
      console.error("Failed to load bazar expenses:", e);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleOpenEdit = (exp: CurryExpense) => {
    setEditingId(exp.id);
    setFormData({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (isNaN(numAmount) || numAmount <= 0) {
      notify("খরচের পরিমাণ ০ এর বেশি হতে হবে।", "error");
      return;
    }

    try {
      const url = editingId ? `/api/curry-expenses/${editingId}` : "/api/curry-expenses";
      ...
      if (res.ok) {
        notify(
          editingId ? "বাজার খরচ সফলভাবে পরিবর্তন হয়েছে!" : "বাজার খরচ যোগ হয়েছে!",
          "success"
        );
        setIsFormOpen(false);
        await loadExpenses();
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/curry-expenses/${deleteTargetId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("adarsha_token")}`,
        },
      ...
      if (res.ok) {
        notify("বাজার খরচ সফলভাবে মুছে ফেলা হয়েছে।", "success");
  ...
  const totalBazarAmount = filteredExpenses.reduce((acc, e) => acc + e.amount, 0);
    ...
    <div className="space-y-5 pb-16">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl sm:text-2xl font-bold text-stone-900 font-serif">
              বাজার খরচ (তরকারি হিসাব)
            </h1>
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800">
              মোট ৳{totalBazarAmount.toLocaleString()}
            </span>
          </div>
          <p className="text-xs text-stone-500 mt-1">
            দৈনিক সবজি, মাছ, মাংস ও মসলা ক্রয়ের হিসাব যা তরকারি রেট তৈরিতে ব্যবহৃত হয়।
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-stone-50 p-3 rounded-2xl border border-stone-200">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="ক্রেতা, ধরন বা বিবরণ দিয়ে খুঁজুন..."
            value={searchQuery}
            ...
            <table className="w-full text-xs text-left border-collapse">
              <thead className="bg-stone-100/90 text-stone-700 border-b border-stone-200">
                <tr>
                  ...
                  <th className="py-3 px-4 font-bold">কিসের বাজার</th>
                  <th className="py-3 px-4 font-bold text-right">খরচ</th>
                  <th className="py-3 px-4 font-bold">নোট / বিবরণ</th>
                  {canManage && (
                ...
                {filteredExpenses.map((exp, idx) => (
                  <tr key={exp.id} className="hover:bg-stone-50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <span className="inline-block px-2 py-0.5 rounded-md text-[11px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200/60">
                        {exp.category}
                      </span>
                    </td>
              ...
              <tfoot className="bg-stone-900 text-white font-bold border-t border-stone-800">
                <tr>
                  ...
                  <td className="py-3 px-4 text-right text-emerald-300 font-mono text-sm">
                    ৳{totalBazarAmount.toLocaleString()}
                  </td>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full p-6 shadow-2xl border border-stone-200 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between pb-3 border-b border-stone-100">
              <h3 className="text-base font-bold text-stone-900">
                {editingId ? "বাজার খরচ সম্পাদনা করুন" : "নতুন বাজার খরচ লিখুন"}
              </h3>
              <button
            ...
            <form onSubmit={handleSave} className="py-4 space-y-3.5">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="যেমন: ৩ কেজি আলু, পেঁপে ও কাঁচা মরিচ"
      ...
      <ConfirmationModal
        ...
        message="আপনি কি এই বাজার খরচের হিসাবটি মুছে ফেলতে চান?"
